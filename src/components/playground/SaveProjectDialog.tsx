...
function SaveProjectDialog() {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-[#121212] border border-[#1e1e1e] p-4 w-full max-w-md animate-fade-in max-h-[70vh] flex flex-col">
        ...
        <div className="flex items-center gap-2 mb-4">
          <input
            ...
            className="flex-1 h-8 bg-[#1a1a1a] border border-[#1e1e1e] px-3 text-xs text-[#c0c0c0] font-mono outline-none focus:border-[#00ff41] placeholder:text-[#606060]"
          />
        ...
        {savedProjects.length > 0 && (
          <>
            <p className="text-[10px] text-[#606060] font-mono mb-2 uppercase tracking-wider">Saved Projects</p>
            <div className="flex-1 overflow-y-auto space-y-1">
              {savedProjects.sort((a, b) => new Date(b.savedAt).getTime() - new Date(a.savedAt).getTime()).map((p) => (
                <div key={p.id} className="flex items-center gap-2 px-2 py-1.5 border border-[#1e1e1e] hover:border-[#2a2a2a] transition-colors group">
                  <button onClick={() => handleLoad(p)} className="flex-1 text-left min-w-0">
                    <p className="text-xs text-[#c0c0c0] font-mono font-bold truncate">{p.name}</p>
                    <p className="text-[10px] text-[#606060] font-mono">
                      <Badge variant="info" className="text-[8px]">{p.language}</Badge>
                      {new Date(p.savedAt).toLocaleDateString()}
                    </p>
                  </button>
                  <button onClick={() => handleDelete(p.id)} className="text-[#606060] hover:text-[#ff3355] opacity-0 group-hover:opacity-100 transition-all">
                    <Trash2 size={12} />
                  </button>
                </div>
              ))}
        ...
        {savedProjects.length === 0 && (
          <p className="text-xs text-[#606060] font-mono text-center py-4">No saved projects yet</p>
        )}
      </div>
    </div>
  )
